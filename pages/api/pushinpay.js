...
export default async function handler(req, res) {
  ...
  // Debug: verificar se o token está sendo lido (não mostrar o valor completo por segurança)
  if (process.env.NODE_ENV === 'development') {
    console.log('🔍 Token PushinPay:', token ? `${token.substring(0, 10)}...` : 'NÃO ENCONTRADO');
  }
  
  if (!token || token === 'SEU_TOKEN_PUSHINPAY_AQUI' || token === 'seu_token_pushinpay_aqui') {
    console.error('❌ Token PushinPay não configurado ou inválido');
    return res.status(500).json({ 
      error: 'Token PushinPay não configurado. Configure a variável PUSHINPAY_TOKEN no .env.local e reinicie o servidor.' 
    });
  }
