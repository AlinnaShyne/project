...
export default function Home() {
  ...
  const [videoCentralPlaying, setVideoCentralPlaying] = useState(false);
  ...
  return (
    <>
      <Head>
        <title>Privacy | Bella Marques</title>
      ...
      <div className="relative max-w-3xl mx-auto rounded-3xl overflow-hidden bg-white shadow-lg mt-4">
        ...
        <div className="bg-white p-6 -mt-12 relative z-10">
          ...
          <div className="flex items-center mb-2 -mt-4">
            <h2 className="text-lg font-bold text-black">Bella Marques</h2> 
            <img className="h-5 w-5 ml-2" src="/images/badge-check.svg" alt="Verificado" />
          </div>
          
          {/* Username */}
          <p className="text-sm text-gray-600 mb-2 -mt-1">@bellamarques</p> 
          
          {/* Description */}
          <p className="text-gray-800 text-sm -mt-2 whitespace-pre-line leading-relaxed">
            💋Oi, eu sou a Bella Marques…{'\n'}
            Uma loira com carinha de anjo e pensamentos que você nem imagina. Aqui eu mostro o meu lado mais ousado.{'\n\n'}
            🍑 Vídeos solo com meu toque especial{'\n'}
            🔥 Momentos acompanhados (do jeitinho que você gosta){'\n'}
            👯 Aventuras com amigas deliciosas{'\n'}
            📱 Chamadas quentes pra quem quer sentir de perto
          </p>
          ...
          <div className="content-transition">
            ...
            <div className="flex items-center mb-6">
              ...
              <div className="flex-1">
                <div className="flex items-center">
                  <h3 className="text-lg font-bold text-gray-900">Bella Marques</h3>
                  <img className="h-5 w-5 ml-2" src="/images/badge-check.svg" alt="Verificado" />
                </div>
                <p className="text-sm text-gray-600">@bellamarques</p>
              </div>
            ...
            <div 
              ...
              onClick={() => {
                const video = document.getElementById('video-central');
                if (video) {
                  if (video.paused) {
                    video.play().then(() => setVideoCentralPlaying(true)).catch(() => {});
                  } else {
                    video.pause();
                    setVideoCentralPlaying(false);
                  }
                }
              }}
            >
              {/* Thumbnail/Preview */}
              <img 
                src="https://i.imgur.com/DKM3dK9.jpg" 
                alt="Vídeo central"
                className={`w-full h-full object-cover transition-opacity duration-300 ${
                  videoCentralPlaying ? 'opacity-0 absolute' : 'opacity-100'
                }`}
                loading="eager"
              />
              ...
              <video 
                id="video-central"
                className={`w-full h-full object-cover transition-opacity duration-300 ${
                  videoCentralPlaying ? 'opacity-100' : 'opacity-0 absolute top-0 left-0'
                }`}
                muted 
                loop
                playsInline
                preload="metadata"
                poster="https://i.imgur.com/DKM3dK9.jpg"
                onPlay={() => setVideoCentralPlaying(true)}
                onPause={() => setVideoCentralPlaying(false)}
              >
                <source src="https://i.imgur.com/DKM3dK9.mp4" type="video/mp4" />
                Seu navegador não suporta vídeos HTML5.
              </video>
              
              {/* Overlay escuro */}
              <div 
                className="absolute inset-0 bg-black transition-opacity duration-300 pointer-events-none rounded-2xl" 
                style={{ 
                  opacity: videoCentralPlaying ? 0.2 : 0.3,
                  background: 'linear-gradient(45deg, rgba(0,0,0,0.3), rgba(255,165,0,0.2))'
                }}
              ></div>
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <svg 
                  className={`w-16 h-16 text-white transition-opacity duration-300 ${
                    videoCentralPlaying ? 'opacity-50' : 'opacity-80'
                  }`} 
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
              ...
              __html: `
                (function() {
                  if (typeof fbq !== 'undefined' && window._fbPixelInitialized) {
                    return; // Já foi inicializado, evitar múltiplos pixels
                  }
